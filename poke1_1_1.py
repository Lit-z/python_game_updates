...
## initial_conditions() function for setting the initial criteria for a fight before callling menu() or menu_bush() function
...
## menu() function (after capturing a 2nd pokemon)
def menu():
    ...
    if opt_menu == "1":
        if user_hp <= 0:
            print("Currently selected pokemon has fainted, please choose a different option (Swap, Heal or Run)")
            menu()
        else:
            battle()
    elif opt_menu == "2":
        ...
        menu()
    return hyper_potion
# has the option to swap to the 2nd pokemon, along with attack/ heal/ run
# this becomes the default menu to appear after catching a 2nd pokemon, if the player never does, then this never does
# future idea: hide attack as an option when current pokemon has 0 hp, temp fixed by denying the player attacking once with a 0 hp pokemon

## menu_bush() is alternative menu function (before catching a 2nd pokemon)
def menu_bush():
    ...
    if opt_menu == "1":
        ...
        print("Choose a number from the options!")
        menu_bush()
# has the option to capture a pokemon (if the fight isn't an npc battle, as stealing pokemon off others would be wrong)
# otherwise the same options of attack/ heal/ run
# this would be the default menu to appear at the start of the game

## capt_pokemon() function (capture pokemon)
def capt_pokemon():
...
# checks for having pokeballs which atm is defaulted to being given 1 during the story
# currently catching is 100% success but later options could be to add scaling/ rng based on the hp of target or different types of pokeballs
# for future expansion of game this variable could potentially be changed to allow for catching more

## battle() function, called for every fight through the menu() or menu_bush() function, unless the player chooses to run (if allowed)
def battle():
...
## type_check() function for working out elemental types for effectiveness calculations
def type_check():
...
## xp_gain() function for calculations after fights
def xp_gain():
    global current_xp, xp_points, current_level
    max_level = 10
    xp_points = random.randint(50,60)
    current_xp += xp_points
    current_level = math.floor(current_xp/ 50) + 1
    if current_level > max_level:
        current_level = max_level
        print(f"You are level {max_level} and can no longer gain xp, so strong")      
    else:
        print(f"Level {current_level}: You gained {xp_points} experience")
    return current_xp, current_level
# XP/ levelling system, with a max level setting and xp values can be adjusted linearly by changing the /
# future thoughts, non linear xp values

## stat_gain() for adjusting stats of players pokemon after they level
def stat_gain():
    global damage, max_hp, current_level
    damage = 5 * (current_level - 1)
    max_hp = 50 + ((current_level - 1) * 20)
    return damage, max_hp
# stat gain system, maybe can be merged into xp function, need to check if both are always called together

## pet_heal() provides a full heal for players pokemon during certain story options
def pet_heal():
    global user_hp, max_hp, pet_one_hp, pet_two_name, pet_two_hp
    user_hp = max_hp
    pet_one_hp = max_hp
    if pet_two_name == " ":
        pass
        ...
        pet_two_hp = max_hp
    return user_hp, pet_one_hp, pet_two_hp
# should be a better way of npc/ dialog heal ups rather than repeated code as a non function
# could be used for a pokemon center option if the game/ story was expanded to make them a choice (atm it happens naturally)
...
-:..     `. `-..--_.,.<       `"      / `.        `-/ |   .
  `,         """"'     `.              ,'         |   |  ',,
   ...
   -" /`.         _,'     | _  _  _.|
    ""--'---"""""'        `' '! |! /
                            `" " - '''
# ascii art for first pokemon choice

## game_start() function, the plot/ what the user will see starts here
def game_start():
    global char_name
...
## difficulty_choice() is a difficulty select that just adds a multiplier into the damage of enemies
...
# mulitpliers of 1, 1.25, 1.5 but could be easily adjusted if required to do so, insane can live upto its name
...
def danger_yes():
    ...
    input()
    print("It is now time to select your Pokemon. You can choose between Squirtle [1], Charmander [2] or Bulbasaur [3].")
    pokemon_select()
...
def pokemon_select():
    ...
    pokemon_choose = pokemon_choose.lower()
    if pokemon_choose == "bulbasaur" or pokemon_choose == "3":
        print("You have chosen the grass type, Bulbasaur! ")
        print(bulb_ascii)
        user_name = "Bulbasaur"
    elif pokemon_choose == "squirtle" or pokemon_choose == "1":
        print("You have chosen the water type, Squirtle! ")
        print(squirtle_ascii)
        user_name = "Squirtle"
    elif pokemon_choose == "charmander" or pokemon_choose == "2":
...
def scientist_save():
    ...
    input()
    print(f'"It seems like that {user_name} has taken a liking to you. You can keep it"')
    input()
    print('"Follow me to my lab i will heal your pokemon up as a thanks. My name is Proffesor Elm. Whats your name friend?"')
    input()
    print(char_name)
    input()
    print(f"You head back to the lab and Prof.Elm heals your {user_name} back to full health")
    input()
    print('"You should give your pokemon a name, what will you call him?"')
    pokemon_name = input(f"Choose {user_name}'s name:\n")
...
def johto_city():
    ...
    input()
    print('"The gym leader, Gus is very tough, and he uses three very strong pokemon, Nidorino, Kadabra and Hitmonchan."')
    input()
...
## endless_loop() function appears after the story
def endless_loop():
...
# is a never ending loop until the player loses if they choose the yes option
# due to the scaling implented they will lose eventually so it records how many fights they manage to succeed at
# as a potential brag/ personal high score challenge for the player
